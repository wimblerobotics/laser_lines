...
LineExtractionROS::LineExtractionROS() : Node("line_finder_node"), data_cached_(false) {
  loadParameters();
  ...
  line_publisher_ = create_publisher<line_finder::msg::LineSegmentList>("line_segments", qos);
  ...
  scan_subscriber_ = create_subscription<sensor_msgs::msg::LaserScan>(
      scan_topic_, qos, std::bind(&LineExtractionROS::laserScanCallback, this, _1));
  if (publish_markers_) {
    marker_publisher_ = create_publisher<visualization_msgs::msg::Marker>("line_markers", qos);
  }

  parameter_callback_handle_ = add_on_set_parameters_callback(
      std::bind(&LineExtractionROS::parametersCallback, this, std::placeholders::_1));
}
...
rcl_interfaces::msg::SetParametersResult LineExtractionROS::parametersCallback(
  ...
  for (const auto &parameter : parameters) {
    if ((parameter.get_name() == "bearing_std_dev") &&
      ...
      RCLCPP_INFO(rclcpp::get_logger("LineExtractionROS"), "Parameter 'frame_id' changed: %s",
                  frame_id_.c_str());
      ...
      RCLCPP_INFO(rclcpp::get_logger("LineExtractionROS"),
                  "Parameter 'least_sq_angle_thresh' changed: %f", least_sq_angle_thresh_);
      ...
      RCLCPP_INFO(rclcpp::get_logger("LineExtractionROS"),
                  "Parameter 'least_sq_radius_thresh' changed: %f", least_sq_radius_thresh_);
    } else if ((parameter.get_name() == "max_line_gap") &&
               (parameter.get_type() == rclcpp::ParameterType::PARAMETER_DOUBLE)) {
      max_line_gap_ = parameter.as_double();
      line_extraction_.setMaxLineGap(max_line_gap_);
      RCLCPP_INFO(rclcpp::get_logger("LineExtractionROS"), "Parameter 'max_line_gap' changed: %f",
                  max_line_gap_);
    } else if ((parameter.get_name() == "max_range") &&
               (parameter.get_type() == rclcpp::ParameterType::PARAMETER_DOUBLE)) {
      max_range_ = parameter.as_double();
      line_extraction_.setMaxRange(max_range_);
      RCLCPP_INFO(rclcpp::get_logger("LineExtractionROS"), "Parameter 'max_range' changed: %f",
                  max_range_);
      ...
      line_extraction_.setMinLinePoints(static_cast<unsigned int>(min_line_points_));
      RCLCPP_INFO(rclcpp::get_logger("LineExtractionROS"),
      ...
      RCLCPP_INFO(rclcpp::get_logger("LineExtractionROS"), "Parameter 'min_range' changed: %f",
                  min_range_);
      ...
      RCLCPP_INFO(rclcpp::get_logger("LineExtractionROS"), "Parameter 'min_split_dist' changed: %f",
                  min_split_dist_);
    } else if ((parameter.get_name() == "outlier_dist") &&
               (parameter.get_type() == rclcpp::ParameterType::PARAMETER_DOUBLE)) {
      outlier_dist_ = parameter.as_double();
      line_extraction_.setOutlierDist(outlier_dist_);
      RCLCPP_INFO(rclcpp::get_logger("LineExtractionROS"), "Parameter 'outlier_dist' changed: %f",
                  outlier_dist_);
      ...
      RCLCPP_INFO(rclcpp::get_logger("LineExtractionROS"), "Parameter 'range_std_dev' changed: %f",
                  range_std_dev_);
    } else if ((parameter.get_name() == "scan_topic") &&
               (parameter.get_type() == rclcpp::ParameterType::PARAMETER_STRING)) {
      scan_topic_ = parameter.as_string();
      RCLCPP_INFO(rclcpp::get_logger("LineExtractionROS"), "Parameter 'scan_topic' changed: %s",
                  scan_topic_.c_str());
    } else {
      RCLCPP_ERROR(rclcpp::get_logger("LineExtractionROS"), "Invalid parameter: %s",
...
void LineExtractionROS::loadParameters() {
  ...
  line_extraction_.setBearingVariance(bearing_std_dev_ * bearing_std_dev_);
  RCLCPP_INFO(rclcpp::get_logger("LineExtractionROS"), "bearing_std_dev: %f", bearing_std_dev_);

  frame_id_ = this->get_parameter("frame_id").as_string();
  RCLCPP_INFO(rclcpp::get_logger("LineExtractionROS"), "frame_id: %s", frame_id_.c_str());

  least_sq_angle_thresh_ = this->get_parameter("least_sq_angle_thresh").as_double();
  line_extraction_.setLeastSqAngleThresh(least_sq_angle_thresh_);
  RCLCPP_INFO(rclcpp::get_logger("LineExtractionROS"), "least_sq_angle_thresh: %f",
              least_sq_angle_thresh_);

  least_sq_radius_thresh_ = this->get_parameter("least_sq_radius_thresh").as_double();
  line_extraction_.setLeastSqRadiusThresh(least_sq_radius_thresh_);
  RCLCPP_INFO(rclcpp::get_logger("LineExtractionROS"), "least_sq_radius_thresh: %f",
              least_sq_radius_thresh_);

  max_line_gap_ = this->get_parameter("max_line_gap").as_double();
  line_extraction_.setMaxLineGap(max_line_gap_);
  RCLCPP_INFO(rclcpp::get_logger("LineExtractionROS"), "max_line_gap: %f", max_line_gap_);

  max_range_ = this->get_parameter("max_range").as_double();
  line_extraction_.setMaxRange(max_range_);
  RCLCPP_INFO(rclcpp::get_logger("LineExtractionROS"), "max_range: %f", max_range_);

  min_line_length_ = this->get_parameter("min_line_length").as_double();
  line_extraction_.setMinLineLength(min_line_length_);
  RCLCPP_INFO(rclcpp::get_logger("LineExtractionROS"), "min_line_length: %f", min_line_length_);

  min_line_points_ = this->get_parameter("min_line_points").as_int();
  line_extraction_.setMinLinePoints(static_cast<unsigned int>(min_line_points_));
  RCLCPP_INFO(rclcpp::get_logger("LineExtractionROS"), "min_line_points: %d", min_line_points_);

  min_range_ = this->get_parameter("min_range").as_double();
  line_extraction_.setMinRange(min_range_);
  RCLCPP_INFO(rclcpp::get_logger("LineExtractionROS"), "min_range: %f", min_range_);

  min_split_dist_ = this->get_parameter("min_split_dist").as_double();
  line_extraction_.setMinSplitDist(min_split_dist_);
  RCLCPP_INFO(rclcpp::get_logger("LineExtractionROS"), "min_split_dist: %f", min_split_dist_);

  outlier_dist_ = this->get_parameter("outlier_dist").as_double();
  line_extraction_.setOutlierDist(outlier_dist_);
  RCLCPP_INFO(rclcpp::get_logger("LineExtractionROS"), "outlier_dist: %f", outlier_dist_);
  ...
  line_extraction_.setRangeVariance(range_std_dev_ * range_std_dev_);
  RCLCPP_INFO(rclcpp::get_logger("LineExtractionROS"), "range_std_dev: %f", range_std_dev_);

  scan_topic_ = this->get_parameter("scan_topic").as_string();
  RCLCPP_INFO(rclcpp::get_logger("LineExtractionROS"), "scan_topic: %s", scan_topic_.c_str());
}
...
void LineExtractionROS::populateClosetPointToLineSegListMsg(
  ...
  for (std::vector<Line>::const_iterator cit = lines.begin(); cit != lines.end(); ++cit) {
    ...
    float qY;
    float d =
        DistanceFromLineSegmentToPoint(cit->getStart()[0], cit->getStart()[1], cit->getEnd()[0],
...
void LineExtractionROS::populateIntersectionMarkers(const std::vector<Line> &lines,
                                                    visualization_msgs::msg::Marker &marker_msg) {
...
///////////////////////////////////////////////////////////////////////////////
void LineExtractionROS::cacheData(const sensor_msgs::msg::LaserScan::ConstSharedPtr scan_msg) {
  const std::size_t num_measurements =
      std::ceil((scan_msg->angle_max - scan_msg->angle_min) / scan_msg->angle_increment);
  std::vector<double> bearings, cos_bearings, sin_bearings;
  std::vector<unsigned int> indices;

  tf2_ros::Buffer tf_buffer = tf2_ros::Buffer(this->get_clock());
  tf2_ros::TransformListener tf_listener(tf_buffer);

  geometry_msgs::msg::TransformStamped transform_stamped;
  try {
    transform_stamped = tf_buffer.lookupTransform("base_link", scan_msg->header.frame_id,
  ...
  for (std::size_t i = 0; i < num_measurements; ++i) {
    ...
    const double b = scan_msg->angle_min + i * scan_msg->angle_increment + yaw;
